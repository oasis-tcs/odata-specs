...
1. _Case where the recursive hierarchy is defined on the input set_  
   ...
   In this case $Π_G(σ(x))$ injects all properties of $x$ into the instances of the output set.
2. _Case where the recursive hierarchy is defined on the related entity type addressed by a navigation property path_  
   ...
   In this case $Π_G(σ(x))$ injects the whole related entity $x$ into the instances of the output set. The navigation property path $p'$ is expanded by default.
3. _Case where the recursive hierarchy is related to the input set only through equality of node identifiers, not through navigation_  
   ...
   In this case $Π_G(σ(x))$ injects only the node identifier of $x$ into the instances of the output set.
...
$R(x)$ is a transformation producing the specified tree order for a sub-hierarchy of $H'$ with root node $x$. Let $c_1,…,c_m$ with $m\ge 0$ be an order-preserving sequence of the children of $x$ in $(H',Q)$.

If $h=\hbox{\tt{preorder}}$, then
$$R(x)=\hbox{\tt{concat}}(F(x)/Π_G(σ(x)),R(c_1),…,R(c_m)).$$

If $h=\hbox{\tt{postorder}}$, then
$$R(x)=\hbox{\tt{concat}}(R(c_1),…,R(c_m),F(x)/Π_G(σ(x))).$$
...
Otherwise $p=p_1/…/p_k/s$ with $k\ge 1$ and
$$\matrix{ 
...
Multiple parents of a node $x$ lead to multiple output set instances that include $σ(x)$. In order to distinguish these, information about the ancestors up to the root is injected into each $σ(x)$ by annotating $x$ differently before each $σ(x)$ is computed.
...
The transformation $Π_G(σ(x))$ is extended with an additional step between steps 2 and 3 of the function $a_G(x,s,p)$:
...
Paths-to-the-root need not have an `Aggregation.UpNode` annotation if they are themselves roots, such as the roots $r_1,…,r_n$ defined above. The transformation $\hbox{\tt{traverse}}(H,Q,p,h,S,o)$ is defined as equivalent to
...
A property $χ_N$ appears in the algorithm, but is not present in the output set. It is explained later (see Example 63). $Z_N$ is a transformation whose output set is its input set with property $χ_N$ removed.
...
$$\matrix{ 
R(x)=\hbox{\tt{concat}}(\hfill\\ 
\quad F(x)/\hbox{\tt{compute}}(x\hbox{\tt{ as }}χ_N)/\hbox{\tt{groupby}}((P_1,P_2),T/Z_N/Π_G(σ(x))),\hfill&\qquad(1)\\ 
...
$$\matrix{ 
R(x)=\hbox{\tt{concat}}(\hfill\\ 
\quad F(x)/\hbox{\tt{compute}}(x\hbox{\tt{ as }}χ_N)/T/Z_N/Π_G(σ(x)),\hfill&\qquad(1)\\ 
\quad R(c_1),…,R(c_m)\hfill&\qquad(2)\\ 
).\hfill 
}$$
...
With $F(x)$ and $c_1,…,c_m$ as above, if at least one of $P_1$ or $P_2$ is non-empty, then
$$\matrix{ 
R(x)=\hbox{\tt{concat}}(\hfill\\ 
\quad F(x)/\hbox{\tt{compute}}(x\hbox{\tt{ as }}χ_N)/\hbox{\tt{groupby}}((P_1,P_2),T/Z_N/Π_G(σ(x))),\hfill\\ 
\quad R(ρ(c_1,r)),…,R(ρ(c_m,r))\hfill\\ 
),\hfill 
}$$
otherwise
$$\matrix{ 
R(x)=\hbox{\tt{concat}}(\hfill\\ 
\quad F(x)/\hbox{\tt{compute}}(x\hbox{\tt{ as }}χ_N)/T/Z_N/Π_G(σ(x)),\hfill\\ 
\quad R(ρ(c_1,r)),…,R(ρ(c_m,r))\hfill\\ 
),\hfill 
}$$
