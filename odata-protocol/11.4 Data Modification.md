...
entity supporting user-specified key values specifies key values 
that do not match an existing entity, then a new entity is created with those
...
is established to that existing entity. If additional updatable properties are 
included for existing entities, the those related entities MUST be updated with 
