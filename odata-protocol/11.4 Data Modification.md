...
obtained from previous requests when updating entities within a collection. If an ETag 
is provided that does not match the ETag value of the entity being updated, or if the entity does not already exist, services that support ETags MUST NOT apply the change and SHOULD indicate a response code of `412 Precondition Failed`. The special value `\*` can be used to match any existing entity but fail if the entity does not already exist.
