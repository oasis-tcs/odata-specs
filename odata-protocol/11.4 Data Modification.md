...
they do not guarantee atomicity. See the [sections ##UpdateaCollectionofEntities],
[##UpdateMembersofaCollection], and [##DeleteMembersofaCollection] below where this preference is mentioned.
...
::: example
Example ##ex: using the JSON format, 4.01 or greater clients can create a new manager
...
::: example
Example ##ex: using the JSON format, 4.01 or greater clients can update the
...
::: example
Example ##ex: using the JSON format, a 4.01 or greater `PATCH` request can update a
...
::: example
Example ##ex: using the JSON format, a 4.01 or greater `PATCH` request can specify a
...
::: example
Example ##ex: When updating an entity with a 4.01 or greater `PUT` request, the target of a
...
##### ##subsubsubsubsec Response Representation of Related Entities
...
in the request payload, then a delta representation of the collection 
containing at least the applied changes SHOULD be included in the response.
...
equivalent to an `If-None-Match: *` header [#OData-JSON#ControlInformationetagodataetag]
