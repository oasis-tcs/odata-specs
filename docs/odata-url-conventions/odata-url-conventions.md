...
::: example
Example 60: all products with a `Name` equal to `Milk` that also have a `Price`
less than 2.55:
```
...
::: example
Example 61: all products that either have a `Name` equal to `Milk` or have a
...
::: example
Example 75: all customers with a `CompanyName` that ends with
`Futterkiste`
```
...
::: example
Example 76: all customers with a `CompanyName` containing `lfreds`
...
::: example
Example 77: all customers with a `CompanyName` that is 19 characters
long
```
...
::: example
Example 79: all customers with a `CompanyName` of `lfreds Futterkiste`
...
::: example
Example 80: all customers with a `CompanyName` that has `lf` as the
...
::: example
Example 81: `hassubset` expressions that return true
```
...
::: example
Example 82: `hassubset` expression that returns false: 1 appears only
...
::: example
Example 85: all customers with a `CompanyName` that match the
...
::: example
Example 89: all customers with a `CompanyName` without leading or
...
::: example
Example 91: all employees born less than 100 milliseconds after a full
...
::: example
Example 92: all employees born in hour 4, between 04:00 (inclusive) and
05:00 (exclusive)
```
...
::: example
Example 95: all employees born in second 40 of any minute of any hour on
any day
```
...
::: example
Example 103: all `Orders` that have any `Items` with a `Quantity` greater
than `100`
```
...
::: example
Example 105: all categories along with their products used in some order
...
::: example
Example 106: all `Orders` that have only `Items` with a `Quantity`
greater than `100`
```
...
::: example
Example 109: check whether a pair of properties has one of several
...
::: example
Example 110: email addresses ending with `.com` assuming
...
::: example
Example 111: customers along with their orders that shipped to the same
...
::: example
Example 112: products with at least 10 positive reviews.
...
::: example
Example 114: products ordered by a set of customers, where the set of
...
::: example
Example 117: similar behavior whether `HeadquarterAddress` is a nullable
...
::: example
Example 119: Return Employees that have any error messages in the
...
::: example
Example 122: all categories and for each category all related products
...
::: example
Example 128: all employees with their manager, manager's manager, and
...
::: example
Example 135: name and description of all products, plus name of expanded
category
```
...
::: example
Example 136: the `AccountRepresentative` property of any supplier that
...
::: example
Example 137: select up to five addresses whose `City` starts with an
...
::: example
Example 138: the `ID` property, the `ActionName` action defined in
...
::: example
Example 139: all products that are blue or green. It is up to the
...
::: example
Example 144: JSON array of strings as parameter alias value --- note that
