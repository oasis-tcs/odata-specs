...
A `204 No Content` response MUST include a `Location` header that can be used to request the result of the completed asynchronous operation.
The `Location` header may or may not have the same value as returned by the previous request.
