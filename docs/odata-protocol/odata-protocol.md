...
These services MUST reference the [`Core`](https://oasis-tcs.github.io/odata-vocabularies/vocabularies/Org.OData.Core.V1.xml) vocabulary in their `$metadata` document,
and clients MUST use either the vocabulary's namespace or the alias defined by the service's reference as the qualifier of `ContentID`.
