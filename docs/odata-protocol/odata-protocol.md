...
request](#UpdateanEntity) to a valid URL that addresses a single entity
and the requested update requires creation of a new entity.
...
(as defined in [section 10](#ContextURL)) can be determined for the entity to be
created. Among other possibilities, this is the case if
- the URL contains the name of an entity set or singleton
- the URL ends with a containment navigation property, optionally followed by a key predicate
- the URL ends with a non-containment navigation property with a navigation property binding,
  optionally followed by a key predicate, or
- the payload includes a context URL.
