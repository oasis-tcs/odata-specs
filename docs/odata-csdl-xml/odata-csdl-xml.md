...
::: {.varxml .example}
Example 82: The collection-if-then-else expression adds "margin-left" or "margin-right" to the collection for left-to-right and right-to-left scripts, respectively, but adds nothing, e.g., for top-to-bottom scripts.
```xml
<Collection>
 <If>
  <Path>IsLTR</Path>
...
::: {.varxml .example}
Example 88: this annotation "morphs" the entity type from [example 13](#entitytype) into
