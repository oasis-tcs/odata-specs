...
Otherwise, if the binding parameter is single-valued, the entity set path has the form
$p/s_1/…/s_k$ with $k>0$.
...
All returned entities MUST belong to the canonical collection of the final navigation
...
6. If $s_i$ names a non-containment navigation property, the service MUST
   ...
   whose path matches $β/s_i$. Let $α'/β'$ be the target of that navigation property
   binding where $α'$ is an entity set or singleton. Update $α=α'$ and $β=β'$.
