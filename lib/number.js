...
class Number {
  constructor(dir, variant, meta) {
    ...
    if (this.meta.funnelweb?.includes("/")) {
      this.funnelweb = new (require(__dirname + "/../" + this.meta.funnelweb))(
        this.meta.name,
      );
      this.codecnt = 0;
    }
  ...
  skip(line) {
    if ((!this.meta.funnelweb || this.funnelweb) && this.skipping !== true) {
        ...
        createInterface({
          ...
          .on(
            ...
            function (line) {
              ...
              if (this.meta.funnelweb && !this.funnelweb)
                line = line
                  .replace(/@!.*$/, "")
                  .replace(/^@[$o]@\<(.*?)@\>.*?@\{/, " ##fwisec $1");
              try {
      ...
      function (resolve, reject) {
        ...
        createInterface({
          ...
          .on(
            ...
            function (line) {
              lineno++;
              if (this.funnelweb && /^@[$o]/.test(line))
                this.funnelweb.code(out, this.codecnt++);
              if (this.skip(line)) return;
              try {
                var extraline;
