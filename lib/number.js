...
class Number {
		...
		return new Promise(function(resolve, reject) {
			createInterface({input: fs.createReadStream(this.dir + "/" + file), crlfDelay: Infinity})
				.on("line", function(line) {
					var m = line.match(/\s##([a-z]+)(_([A-Za-z]*))?/);
					if (m) {
	...
	phase2(file, out) {
		return new Promise(function(resolve, reject) {
			createInterface({input: fs.createReadStream(this.dir + "/" + file), crlfDelay: Infinity})
				.on("line", function(line) {
					out.write(line.replace(/(\s)##[a-z]+(_[A-Za-z]+)?/, function(m, p) {
						return p + this.number[this.match++];
					}.bind(this)).replace(/\(##_([A-Za-z]+)\)/g, function(m, p) {
						var r = this.refs[p];
						if (r) return `(${r.href})`;
						else return `UNRESOLVED ${p}`;
					}.bind(this)).replace(/(\s)##_([A-Za-z]*)\]/g, function(m, p, q) {
						var r = this.refs[q];
