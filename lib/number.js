...
class Number {
  ...
  skip(line) {
    if (this.funnelweb && this.skipping !== true) {
      const { skipping, skip } = this.funnelweb.skip(line) || {};
      if (skipping !== undefined) this.skipping = skipping;
      if (skip) return true;
    }
    if (this.fence) {
      if (line.trimEnd() === this.fence) {
        const skip = this.skipping || this.fence.length < 3;
    ...
    const m = line.match(/^(:+)\s+(?:\{\.)?(var(\S+)|funnelweb)/);
    if (m) {
      this.fence = m[1];
      if (
      ...
      function (resolve, reject) {
        ...
        createInterface({
          ...
          .on(
            ...
            function (line) {
              lineno++;
              if (this.funnelweb) this.funnelweb.code(out);
              if (this.skip(line)) return;
              try {
                var extraline;
