...
class Number {
	...
	secno(type) {
		return type === "asec" ? String.fromCharCode(64 + this.counter[type]) : String(this.counter[type]);
	}
				...
				.on("line", function(line) {
					...
					if (m && line[m.index + m[0].length] !== "]") {
						...
						for (var supc = m[1]; supc.startsWith("sub");)
							this.number[this.match] = this.secno(supc = supc.substring(3)) + "." + this.number[this.match];
						if (m[1].endsWith("sec")) {
						...
						if (m[2]) {
							...
							this.number[this.match] = `<a name="${m[3]}" href="#${m[3]}">${this.number[this.match]}${
								m[1] === "asec" ? "." : ""}</a>`;
						}
						this.match++;
					}
	...
	phase2(file, out) {
		return new Promise(function(resolve, reject) {
			...
			createInterface({input: fs.createReadStream(this.dir + "/" + file), crlfDelay: Infinity})
				.on("line", function(line) {
					lineno++;
					line = line.replace(/\$\$\$(.*?)\$\$\$/g, function(m, p) {
						return this.meta[p] || m;
					}.bind(this));
					for (var m, regex = /\]\(#(.*?)\)/g; m = regex.exec(line);)
						if (!this.refs[m[1]])
							this.errors.push(`Undefined link #${m[1]} in file ${this.dir + "/" + file}, line ${lineno}`);
					m = line.match(/ ##([A-Za-z]+)(_([A-Za-z]+))?/);
					if (m && line[m.index + m[0].length] !== "]") {
						var prefix = line.substring(0, m.index) + " ";
						var suffix = line.substring(m.index + m[0].length);
						if (m[1].endsWith("sec"))
							line = (prefix + this.number[this.match].replace(/<\/a>$/, suffix + "</a>")).replace(/(Appendix )(<a.*?>)/, "$2$1");
						else
							line = prefix + this.number[this.match] + suffix;
						this.match++;
					}
					out.write(line.replace(/##([A-Za-z]+)\]/g, function(m, p) {
						...
						if (r) return `${r}](#${p})`;
							...
							this.errors.push(`Undefined link ##${p} in file ${this.dir + "/" + file}, line ${lineno}`);
							return `~~${p}~~]`;
						}
					}.bind(this)));
