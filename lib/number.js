...
class Number {
  ...
  secno(type) {
    var t = type.match(/^(sub)*(.*?)isec$/);
    return type === "asec"
      ? String.fromCharCode(64 + this.counter[type])
      : (t?.[2] || "") + this.counter[type];
  }

  skip(line) {
    if (
      (!this.meta.funnelweb || this.meta.funnelweb.includes(".")) &&
      this.skipping !== true
    ) {
    ...
    if (m) {
      ...
      if (
        ...
        (m[2] === "funnelweb" && !this.meta.funnelweb)
      )
        ...
        createInterface({
          ...
          .on(
            ...
            function (line) {
              ...
              if (this.meta.funnelweb && !this.meta.funnelweb.includes("."))
                line = line
                  .replace(/@!.*$/, "")
                  .replace(/^@[$o]@\<(.*?)@\>.*?@\{/, " ##fwisec $1");
              try {
                ...
                if (m && line[m.index + m[0].length] !== "]") {
                  ...
                  if (m[2]) {
                    ...
                    this.number[this.match] = `<a id="${m[3]}" href="#${
                      m[3]
                    }">${
      ...
      function (resolve, reject) {
        ...
        createInterface({
          ...
          .on(
            ...
            function (line) {
              lineno++;
              if (this.meta.funnelweb?.includes(".") && /^@[$o]/.test(line))
                out.write(
                  "::: funnelweb\n" +
                    (this.code[this.codecnt++] || "").replace(
                      /<(\/?TABLE|TR|TD|FONT COLOR).*?>/g,
                      "",
                    ) +
                    "</P>\n:::\n",
                );
              ...
              try {
                ...
                if (m1)
                  ...
                  if (extraline) out.write(extraline + "\n");
                }
              } catch (e) {
                this.errors.push(`${file}#${lineno}: ${e.toString()}`);
              }
  ...
  async write(out) {
    if (this.meta.funnelweb?.includes(".")) {
      this.code = fs
        .readFileSync(__dirname + "/../" + this.meta.funnelweb)
        .toString()
        .match(/<P>.*?(?=<P>)/gs);
      this.codecnt = 0;
    }
