...
class Number {
  ...
  skip(line) {
    ...
    if (m) {
      ...
      if (
        ...
        (m[2] === "funnelweb" && !this.meta.funnelweb && !this.meta.funnelweave)
      )
      ...
      function (resolve, reject) {
        ...
        createInterface({
          ...
          .on(
            ...
            function (line) {
              lineno++;
              if (this.meta.funnelweave && /^@[$o]/.test(line))
                out.write(
                  "::: funnelweb\n" +
  ...
  async write(out) {
    if (this.meta.funnelweave) {
      this.code = fs
        .readFileSync(__dirname + "/../" + this.meta.funnelweave)
        .toString()
        .match(/<P>.*?(?=<P>)/gs);
      this.codecnt = 0;
    }
