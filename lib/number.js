...
class Number {
  constructor(dir, variant, meta) {
    ...
    this.chapters = fs
      .readdirSync(`${__dirname}/../${this.dir}`)
      .filter((fn) => fn.endsWith(".md"))
      .sort(compareSectionNumbers);
    this.meta =
      meta ||
      yaml.load(
        fs.readFileSync(`${__dirname}/../${this.dir}/${this.variant || "meta"}.yaml`),
      );
  ...
  phase1(file) {
    return new Promise(
      function (resolve, reject) {
        ...
        var lineno = 0;
        createInterface({
          input: fs.createReadStream(`${__dirname}/../${this.dir}/${file}`),
          crlfDelay: Infinity,
        })
          .on(
            "line",
          ...
          .on(
            ...
            function (line) {
              ...
              try {
                ...
                if (m1) {
                  out.write(
                    fs.readFileSync(`${__dirname}/../${this.dir}/${m1[1]}`),
                  );
                  return;
                }
