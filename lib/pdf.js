...
module.exports = async function (name, _variant, meta, force) {
  if (!force) {
    const stylesDir = `docs/${name}/styles`;
    const htmlFile = `docs/${name}/${name}.html`;
    const pdfFile = `docs/${name}/${name}.pdf`;
  ...
  if (force) {
    ...
    const htmlUrl = url.pathToFileURL(`docs/${name}/${name}.html`).href;
