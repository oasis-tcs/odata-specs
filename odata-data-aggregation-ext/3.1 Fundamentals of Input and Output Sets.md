...
- Instances in an output set that are unaggregated entities have the metadata associated with that entity, such as entity-id, read and edit URL (defined in [#OData-Protocol#ServiceModel]) and ETag (defined in [#OData-Protocol#UseofETagsforAvoidingUpdateConflicts]) as well as relations to other entities [#OData-Protocol#RequestingRelatedEntities]. Such instances are
  - copied from the input set by a non-aggregating transformation (as defined in subsections [##TransformationsProducingaSubset], [##OnetoOneTransformations], and [##TransformationsChangingtheInputSetStructure]) or
  - copied from the input set during grouping by a navigation property $q$ (through the operation $u[q]=s[q]$ in step 2, second bullet point, of the definition of $a_G(u,s,p)$ in [section ##SimpleGrouping]).
