...
- Unaggregated entities in an output set that are copied from the input set have the metadata associated with them, such as entity-id, read and edit URL (defined in [#OData-Protocol#ServiceModel]) and ETag (defined in [#OData-Protocol#UseofETagsforAvoidingUpdateConflicts]) as well as relations to other entities [#OData-Protocol#RequestingRelatedEntities]. Such instances are copied from the input set
  - by a non-aggregating transformation listed in section [##TransformationsProducingaSubset], [##OnetoOneTransformations], or [##TransformationsChangingtheInputSetStructure] or
  - by a transformation ${\tt groupby}((q),…)$ for a navigation property $q$ as defined in [section ##SimpleGrouping].
