...
The system query option `$apply` is evaluated first, then the other system query options are evaluated, if applicable, on the result of `$apply`, see [OData-Protocol, section "System Query Options"]($$$OData-Protocol$$$#SystemQueryOptions). Stability across requests for system query options `$top` and `$skip` [OData-Protocol, sections "`$top`" and "`$skip`"]($$$OData-Protocol$$$#SystemQueryOptiontop) is defined in [section ##StableTotalOrderBeforeskipandtop].
