...
- Instances in an output set that are the result of only non-aggregating transformations (as defined in subsections [##TransformationsProducingaSubset], [##OnetoOneTransformations], and [##TransformationsChangingtheInputSetStructure]) have the metadata associated with that entity, such as entity-id, read and edit URL (defined in [#OData-Protocol#ServiceModel]) and ETag (defined in [#OData-Protocol#UseofETagsforAvoidingUpdateConflicts]) as well as relations to other entities [#OData-Protocol#RequestingRelatedEntities].
